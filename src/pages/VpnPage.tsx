...
export default function VpnPage() {
  const fetchVpn = useCallback(() => fetch(`${API}/vpn`).then(r => r.json()), []);
  ...
  const handleSetup = useCallback(async () => {
    await fetch(`${API}/vpn/setup`, { method: 'POST' });
    refresh();
  }, [refresh]);

  const handleAddPeer = useCallback(async () => {
    if (!peerName.trim()) return;
    const res = await fetch(`${API}/vpn/peers`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: peerName.trim() }) });
  ...
  const handleDeletePeer = useCallback(async (id: string) => {
    await fetch(`${API}/vpn/peers/${id}`, { method: 'DELETE' });
    refresh();
  }, [refresh]);
