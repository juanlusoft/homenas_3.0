...
const pendingTokens = new Map<string, { id: string; name: string; os: string; backupType: 'full' | 'folders' }>();
...
activeBackupRouter.get('/agent/generate/:platform', requireAdmin, (req, res) => {
  try {
    ...
    // (no pre-registration — pending entry only appears when agent actually connects)
    pendingTokens.set(token, {
      id,
      name: customName || `${platform}-${id}`,
      os: platform === 'windows' ? 'Windows' : platform === 'mac' ? 'macOS' : 'Linux',
      backupType: backupType as 'full' | 'folders',
    });
...
activeBackupRouter.post('/agent/activate', (req, res) => {
  const { token, hostname, os: agentOS } = req.body;
  if (!token || !hostname) return res.status(400).json({ error: 'token and hostname required' });

  const ip = (req.headers['x-forwarded-for'] as string)?.split(',')[0]?.trim() || req.ip || 'unknown';
  const authToken = crypto.randomBytes(32).toString('hex');

  // Already activated — just refresh auth token
  const existing = Array.from(devices.values()).find(d => d.hostname === hostname && d.ip === ip);
  if (existing) {
    existing.token = authToken;
    existing.lastSeen = new Date().toISOString();
    return res.json({ deviceID: existing.id, authToken });
  }
  ...
  // Create pending entry — admin must approve before backups start
  const device: Device = {
    id,
    name: deviceName,
    hostname,
...
activeBackupRouter.post('/pending/:id/reject', requireAdmin, (req, res) => {
  ...
  devices.delete(id); // also remove the pre-created device entry
  saveData();
  res.json({ success: true });
});
