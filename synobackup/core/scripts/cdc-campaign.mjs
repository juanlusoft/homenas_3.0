...
async function api(path, { method = 'GET', auth, headers = {}, body } = {}) {
...
function makeBaseBuffer(size, seed) {
...
function scenarioBuffers(base) {
  return {
    prepend64k: Buffer.concat([Buffer.alloc(64 * 1024, 0x7a), base]),
    insert128k: Buffer.concat([
      base.subarray(0, Math.floor(base.length / 2)),
      Buffer.alloc(128 * 1024, 0x5c),
      base.subarray(Math.floor(base.length / 2)),
    ]),
    modify128k: (() => {
      const copy = Buffer.from(base);
      copy.fill(0x33, 5 * 1024 * 1024, 5 * 1024 * 1024 + 128 * 1024);
      return copy;
    })(),
    append64k: Buffer.concat([base, Buffer.alloc(64 * 1024, 0x6b)]),
  };
}

function fixedPlan(buffer) {
  const chunks = [];
  for (let offset = 0; offset < buffer.length; offset += FIXED_CHUNK_SIZE) {
    const end = Math.min(offset + FIXED_CHUNK_SIZE, buffer.length);
    const part = buffer.subarray(offset, end);
    chunks.push({ hash: sha256(part), start: offset, end, size: part.length });
  }
  return chunks;
}

function cdcPlan(buffer) {
  ...
  for (let i = 0; i < buffer.length; i++) {
    ...
    if ((hash & mask) === 0 || size >= CDC_MAX) {
      ...
      chunks.push({ hash: sha256(part), start, end, size: part.length });
      start = end;
      hash = 0;
      windowCount = 0;
    }
  ...
  if (start < buffer.length) {
    ...
    chunks.push({ hash: sha256(part), start, end: buffer.length, size: part.length });
  }
  return chunks;
}

async function uploadChunk(deviceId, authToken, sessionId, chunk) {
  const response = await fetch(`${baseUrl}/api/synobackup/agent/${deviceId}/sessions/${sessionId}/chunks/${chunk.hash}`, {
    method: 'PUT',
    headers: { Authorization: `Bearer ${authToken}` },
    body: chunk.body,
  });
  if (response.status >= 400) {
    throw new Error(`chunk upload failed ${chunk.hash}: ${response.status} ${await response.text()}`);
  }
}

async function commitFile(deviceId, authToken, sessionId, relativePath, buffer, chunks, modifiedAt) {
  const descriptor = {
    path: relativePath,
    size: buffer.length,
    modifiedAt,
    ...
    chunkSize: CDC_AVG,
    chunkSha256: chunks.map((chunk) => chunk.hash),
    chunkBytes: chunks.map((chunk) => chunk.size),
  };
  const probe = await api(`/api/synobackup/agent/${deviceId}/sessions/${sessionId}/files/probe`, {
    method: 'POST',
    auth: authToken,
    body: descriptor,
  });
  const missing = new Set(probe.missingChunks || []);
  let uploadedBytes = 0;
  for (const chunk of chunks) {
    if (!missing.has(chunk.hash)) continue;
    await uploadChunk(deviceId, authToken, sessionId, {
      hash: chunk.hash,
      body: buffer.subarray(chunk.start, chunk.end),
    });
    uploadedBytes += chunk.size;
  }
  const commit = await api(`/api/synobackup/agent/${deviceId}/sessions/${sessionId}/files/commit`, {
    method: 'POST',
    auth: authToken,
    body: descriptor,
  });
  return { probe, commit, uploadedBytes, totalBytes: buffer.length, chunkCount: chunks.length };
}
...
async function createDevice(label) {
  ...
  await admin(`/api/synobackup/admin/devices/${activated.deviceId}/retention-policy`, {
    method: 'PUT',
    body: { keepLast: 1, keepDaily: 0, keepWeekly: 0, keepMonthly: 0 },
  });
  return activated;
}

async function startSession(deviceId, authToken, volume) {
  return api(`/api/synobackup/agent/${deviceId}/sessions/start`, {
    method: 'POST',
    auth: authToken,
    body: { volume, snapshotPath: `campaign-${volume}` },
  });
}

async function completeSession(deviceId, authToken, sessionId) {
  return api(`/api/synobackup/agent/${deviceId}/sessions/${sessionId}/complete`, {
    method: 'POST',
    auth: authToken,
    body: { success: true },
  });
...
async function runMode(mode, planner, seed) {
  const activated = await createDevice(`campaign-${mode}-${crypto.randomUUID().slice(0, 6)}`);
  const base = makeBaseBuffer(32 * 1024 * 1024, seed);
  const scenarios = scenarioBuffers(base);

  const baseSession = await startSession(activated.deviceId, activated.authToken, `${mode}-base`);
  const baseChunks = planner(base);
  const baseUpload = await commitFile(
    activated.deviceId,
    activated.authToken,
    baseSession.sessionId,
    `campaign/${mode}/base.bin`,
    base,
    baseChunks,
    new Date().toISOString(),
  );
  await completeSession(activated.deviceId, activated.authToken, baseSession.sessionId);

  const results = [];
  for (const [scenarioName, scenarioBuffer] of Object.entries(scenarios)) {
    const session = await startSession(activated.deviceId, activated.authToken, `${mode}-${scenarioName}`);
    const chunks = planner(scenarioBuffer);
    const outcome = await commitFile(
      activated.deviceId,
      activated.authToken,
      session.sessionId,
      `campaign/${mode}/${scenarioName}.bin`,
      scenarioBuffer,
      chunks,
      new Date().toISOString(),
    );
    await completeSession(activated.deviceId, activated.authToken, session.sessionId);
    results.push({
      scenario: scenarioName,
      totalBytes: outcome.totalBytes,
      uploadedBytes: outcome.uploadedBytes,
      savedBytes: outcome.totalBytes - outcome.uploadedBytes,
      knownChunks: outcome.probe.knownChunks,
      missingChunks: (outcome.probe.missingChunks || []).length,
      chunkCount: outcome.chunkCount,
    });
  }

  await admin(`/api/synobackup/admin/devices/${activated.deviceId}/retention/run?dryRun=false`, {
    method: 'POST',
    body: {},
  });
  ...
  return {
    ...
    baseUploadedBytes: baseUpload.uploadedBytes,
    baseLogicalBytes: baseUpload.totalBytes,
...
async function main() {
  await api('/api/synobackup/health');
  const fixedSeed = crypto.randomInt(0, 0xffffffff);
  let cdcSeed = crypto.randomInt(0, 0xffffffff);
  while (cdcSeed === fixedSeed) {
    cdcSeed = crypto.randomInt(0, 0xffffffff);
  }
  const fixed = await runMode('fixed', fixedPlan, fixedSeed);
  const cdc = await runMode('cdc', cdcPlan, cdcSeed);

  const comparison = cdc.scenarios.map((item) => {
    const fixedScenario = fixed.scenarios.find((entry) => entry.scenario === item.scenario);
    return {
      scenario: item.scenario,
      fixedUploadedBytes: fixedScenario?.uploadedBytes ?? null,
      cdcUploadedBytes: item.uploadedBytes,
      improvementBytes: (fixedScenario?.uploadedBytes ?? 0) - item.uploadedBytes,
      fixedChunkCount: fixedScenario?.chunkCount ?? null,
      cdcChunkCount: item.chunkCount,
    };
  });
